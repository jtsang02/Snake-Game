...
"""
import threading
import queue        # thread-safe queue from Python standard library
...
class Game():
    ...
    def superloop(self) -> None:
        """
            ...
            The SPEED constant sets how often the move tasks
            are generated.
        """
        SPEED = 0.15  # speed of snake updates (sec)
        while self.gameNotOver:
            self.move()                                         # call move method
            # add task to queue to move
    ...
    def move(self) -> None:
        ...
        if (self.rectangleCoordinates[0] == NewSnakeCoordinates[0] and
            ...
            self.score += 1
            # add task to queue to update score
    ...
    def createNewPrey(self) -> None:
        """ 
            ...
            away from the walls. 
        """
        THRESHOLD = 15  # set how close prey can be to borders
        # create random x coordinate
        x = random.randrange(THRESHOLD, WINDOW_WIDTH - 5, 10)
        # create random y coordinate
        y = random.randrange(THRESHOLD, WINDOW_HEIGHT - 5, 10)
        self.rectangleCoordinates = x, y
        # add prey task to queue with value as rectangle coordinates
        self.queue.put({"prey": (x - 5, y - 5, x + 5, y + 5)})
...
if __name__ == "__main__":
    ...
    QueueHandler(gameQueue, gui)  # instantiate queue handler
